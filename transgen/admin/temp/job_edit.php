...
$sql1 = "select * from `".$dbpre."job` where id='" . $id . "'";
...
switch ($action) {
    ...
    case"edit";
	 ...
	 $addtime = strtotime($_POST['addtime']);
        ...
        $sql = "update `".$dbpre."job` set name='$name',renshu='$renshu',area='$area',title='$title',keywords='$keywords',description='$description',zhize='$zhize',yaoqiu='$yaoqiu',addtime='$addtime' where id='".$id."'";
		if(mysql_query($sql, $conn)){
        	echo "<script>alert('修改成功');window.location.href='job_edit.php?id=".$id."'</script>";
		}else{
    ...
    <form name="myform" id="myform" action="?action=edit" method="post" onsubmit="return checkform(this)">
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          ...
          <td align="left" class="bline"><input name="renshu" type="text" id="renshu" style="width:100px;" value="<?php echo $rs['renshu'];?>" /></td>
        </tr>
        <tr>
          <td height="40" align="center" class="bline">招聘地区</td>
          <td align="left" class="bline"><input name="area" type="text" id="area" style="width:100px;" value="<?php echo $rs['area'];?>" /></td>
        </tr>
        <tr>
          <td width="13%" height="40" align="center" class="bline">title</td>
          <td width="87%" align="left" class="bline"><input name="title" type="text" id="title" value="<?php echo $rs['title'];?>" style="width:400px;" />
            仅SEO优化使用</td>
        </tr>
        <tr>
...
仅SEO优化使用</td>
    	...
    	<textarea id="zhize" name="zhize" style="width:100%;height:300px;visibility:hidden;"><?php echo $rs['zhize'];?></textarea>
        </div>
        ...
        <textarea id="yaoqiu" name="yaoqiu" style="width:100%;height:300px;visibility:hidden;"><?php echo $rs['yaoqiu'];?></textarea>
        </div>
          </td>
        </tr>
        <tr>
