...
if(!in_array('17',$quanxian)){
    ...
    switch ($action) {
        ...
        case"add";
            ...
            if ($zipa["type"]!=''){
                $patha = "../attached/down/" . time()."_".rand(111111,999999) . "." . $fileExa;
                move_uploaded_file($zipa["tmp_name"], $patha);
            }else{
                $patha = $u;
            }
            if ($zipb["type"]!=''){
                $pathb = "../attached/down/" . time()."_".rand(111111,999999) . "." . $fileExb;
                move_uploaded_file($zipb["tmp_name"], $pathb);
            }else{
                $pathb = $u;
            }
            if ($zipc["type"]!=''){
                $pathc = "../attached/down/" . time()."_".rand(111111,999999) . "." . $fileExc;
                move_uploaded_file($zipc["tmp_name"], $pathc);
            }else{
                $pathc = $u;
            }
            if ($zipd["type"]!=''){
                $pathd = "../attached/down/" . time()."_".rand(111111,999999) . "." . $fileExd;
