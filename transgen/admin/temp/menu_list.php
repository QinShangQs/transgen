...
switch ($action) {
    ...
    case"del";
        ...
        if (empty($_POST['id'])) {
            ...
            $str = "delete from `".$dbpre."menu` where id in ($id)";
            mysql_query($str);
            echo "<script>alert('删除成功！');window.location.href='menu_list.php?page=$page';</script>";
        }
        break;
}
?>
        ...
        <form id="form2" name="form2" method="post" action="?action=del">
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <td width="53%" align="left" class="bline">菜单名称</td>
                    <td width="24%" align="center" class="bline">链接地址</td>
                    <td width="13%" align="center" class="bline">相关操作</td>
                </tr>
                <?php
               $sqlzt = "select * from `".$dbpre."menu` where parent_id=0 order by id asc";
                ...
                while ($rslist = mysql_fetch_array($queryzt)) {
                    ...
                    <tr bgcolor="#ffffff" onmouseover="this.style.background='#EEFAFF'; "
                        ...
                        <td align="left" class="bline"><b><?php echo $rslist['menu_name']; ?></b></td>
                        <td align="center" class="bline"></td>
                        <td align="center" class="bline"><a href="menu_edit.php?id=<?php echo $rslist['id']; ?>">编辑</a>                        </td>
                    </tr>
                    <?php
					$sqlsm = "select * from `".$dbpre."menu` where parent_id='".$rslist['id']."' order by id asc";
					$querysm = mysql_query($sqlsm, $conn);
					while ($rssm = mysql_fetch_array($querysm)) {
						?>
                    <tr bgcolor="#ffffff" onmouseover="this.style.background='#EEFAFF'; "
                        onmouseout="this.style.background='#ffffff'; this.style.bordercolor=''">
                        <td height="30" align="center" class="bline">
                            <label>
                                <input type="checkbox" name="id[]" value="<?php echo $rssm['id']; ?>"
                                       style="background:none; border:none;"/>
                            </label>                        </td>
                        <td align="left" class="bline"><?php echo $rssm['id']; ?></td>
                        <td align="left" class="bline">|————<?php echo $rssm['menu_name']; ?></td>
                        <td align="center" class="bline"><?php echo $rssm['url']; ?></td>
                        <td align="center" class="bline"><a href="menu_edit.php?id=<?php echo $rssm['id']; ?>">编辑</a>                        </td>
                    </tr>
					<?php
                        }
                    ?>
