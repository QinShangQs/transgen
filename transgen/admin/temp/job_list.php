...
switch($action){
    ...
    case"del";
		...
		$str="delete from `".$dbpre."job` where id in ($id)";
		mysql_query($str);
		echo "<script>alert('删除成功！');window.location.href='job_list.php?PB_page=$pg';</script>";
		}
        break;
}
?>
...
<input type="hidden" id="pg" name="pg" value="<?php echo $_GET['PB_page'];?>" />   
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
          <tr>
              ...
              <td width="41%" align="left" class="bline">招聘职位</td>
              <td width="13%" align="center" class="bline">人数</td>
              <td width="13%" align="center" class="bline">地区</td>
          ...
          <?php
			$cat = $_GET['cat'];
			$Page_size =12;		
			$sqld = "SELECT id FROM `".$dbpre."job` where 1=1";
			if($cat){
				$sqld.= " and cat='".$cat."'";
			}
			...
			$sqlzt = "select id,name,renshu,area,addtime from `".$dbpre."job` where 1=1";
			if($cat){
				$sqlzt.= " and cat='".$cat."'";
			}
			...
			?>					
              <tr bgcolor="#ffffff" onmouseover="this.style.background='#EEFAFF'; " onmouseout ="this.style.background='#ffffff'; this.style.bordercolor=''">
                ...
                <input type="checkbox" name="id[]" value="<?php echo $rslist['id'];?>" style="background:none; border:none;" />
                  ...
                  <td align="center" class="bline"><?php echo $rslist['renshu']; ?></td>
                  <td align="center" class="bline"><?php echo $rslist['area']; ?></td>
                  <td align="center" class="bline"><?php echo  date('Y-m-d H:i:s', $rslist['addtime']);?></td>
                  <td align="center" class="bline"><a href="job_edit.php?id=<?php echo $rslist['id']; ?>">编辑</a></td>
              </tr>	
             <?php
			}
  ...
  <input type="submit" name="btnSave" value="删除"/></div>
           ...
           <tr bgcolor="#ffffff">
                <td height="30" colspan="7" align="center"><?php if($count >0){ echo $key;}else{echo "暂无内容";}?></td>
           </tr>	
    </table>
    </form>
  </DIV>
