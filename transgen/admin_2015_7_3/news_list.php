...
switch($action){
    ...
    case"del";
		...
		$str="delete from `".$dbpre."news` where id in ($id)";
		mysql_query($str);
		echo "<script>alert('删除成功！');window.location.href='news_list.php?page=$pg';</script>";
		}
        break;
}
?>
...
<input type="hidden" id="pg" name="pg" value="<?php echo $_GET['PB_page'];?>" />   
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
          <tr>
              ...
              <td width="41%" align="left" class="bline">栏目名称</td>
              <td width="18%" align="center" class="bline">所属栏目</td>
              <td width="8%" align="center" class="bline">浏览次数</td>
          ...
          <?php
			$cat = $_GET['cat'];
			$Page_size =12;		
			$sqld = "SELECT * FROM `".$dbpre."news` where 1=1";
			if($cat){
				$sqld.= " and cat='".$cat."'";
			}
			...
			$sqlzt = "select * from `".$dbpre."news` where 1=1";
			if($cat){
				$sqlzt.= " and cat='".$cat."'";
			}
			...
			?>					
              <tr bgcolor="#ffffff" onmouseover="this.style.background='#EEFAFF'; " onmouseout ="this.style.background='#ffffff'; this.style.bordercolor=''">
                ...
                <input type="checkbox" name="id[]" value="<?php echo $rslist['id'];?>" style="background:none; border:none;" />
                  ...
                  <?php
                  $sqlcat = mysql_query("select id,catname from `".$dbpre."catnews` where id='".$rslist['cat']."'",$conn);
				  $rowcat = mysql_fetch_array($sqlcat);
				  echo $rowcat['catname'];
				  ?>
                  </td>
                  <td align="center" class="bline"><?php echo $rslist['hist']; ?></td>
                  <td align="center" class="bline"><?php echo  date('Y-m-d H:i:s', $rslist['addtime']);?></td>
                  <td align="center" class="bline"><a href="news_edit.php?id=<?php echo $rslist['id']; ?>">编辑</a></td>
              </tr>	
             <?php
			}
