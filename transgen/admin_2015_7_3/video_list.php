...
switch($action){
    ...
    case"del";
		...
		$str="delete from `".$dbpre."video` where id in ($id)";
		mysql_query($str);
		echo "<script>alert('删除成功！');window.location.href='video_list.php?PB_page=$pg';</script>";
		}
        break;
}
?>
...
<input type="hidden" id="pg" name="pg" value="<?php echo $_GET['PB_page'];?>" />   
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
          <tr>
              ...
              <td width="66%" align="left" class="bline">视频名称</td>
              <td width="17%" align="center" class="bline">发布时间</td>
              <td width="8%" align="center" class="bline">相关操作</td>
          </tr>
          <?php
			$cat = $_GET['cat'];
			$Page_size =12;		
			$sqld = "SELECT id FROM `".$dbpre."video` where 1=1";
			if($cat){
				$sqld.= " and cat='".$cat."'";
			}
			...
			$sqlzt = "select id,name,addtime from `".$dbpre."video` where 1=1";
			if($cat){
				$sqlzt.= " and cat='".$cat."'";
			}
			...
			?>					
              <tr bgcolor="#ffffff" onmouseover="this.style.background='#EEFAFF'; " onmouseout ="this.style.background='#ffffff'; this.style.bordercolor=''">
                ...
                <input type="checkbox" name="id[]" value="<?php echo $rslist['id'];?>" style="background:none; border:none;" />
                  ...
                  <td align="center" class="bline"><a href="video_edit.php?id=<?php echo $rslist['id']; ?>">编辑</a></td>
              </tr>	
             <?php
			}
