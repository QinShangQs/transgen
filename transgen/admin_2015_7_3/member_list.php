...
switch($action){
    ...
    case"member_reset_password";
	 ...
	 $id = trim($_GET['id']);
	 $password = '96e79218965eb72c92a549dd5a330112';//默认密码为11111
		
        $sqle = "update `".$dbpre."member` set password='$password' where id='" . $id . "'";
		if(mysql_query($sqle, $conn)){
        	echo "<script>alert('密码重置成功');window.location.href='member_list.php'</script>";
		}else{
			echo "<script>alert('密码重置失败');window.location.href='javascript:history.go(-1)'</script>";
		}
	break;
    ...
    case"del";
		...
		$str="delete from `".$dbpre."member` where id in ($id)";
		mysql_query($str);
		echo "<script>alert('删除成功！');window.location.href='member_list.php?PB_page=$pg';</script>";
		}
        break;
}
?>
...
<input type="hidden" id="pg" name="pg" value="<?php echo $_GET['PB_page'];?>" />   
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
          <tr>
              <td width="2%" height="30" align="center" class="bline">ID</td>
              <td width="4%" align="left" class="bline">&nbsp;</td>
              <td width="18%" align="left" class="bline">会员名</td>
              <td width="16%" align="center" class="bline">手机号码</td>
              <td width="23%" align="center" class="bline">邮箱</td>
              <td width="16%" align="center" class="bline">注册时间</td>
            <td width="12%" align="center" class="bline">相关操作</td>
              <td width="9%" align="center" class="bline">查看详情</td>
          </tr>
          <?php
			$cat = $_GET['cat'];
			$Page_size =12;		
			$sqld = "SELECT * FROM `".$dbpre."member` order by id desc";
			...
			$sqlzt = "select * from `".$dbpre."member` order by addtime desc limit $offset,$Page_size";
			...
			?>					
              <tr bgcolor="#ffffff" onmouseover="this.style.background='#EEFAFF'; " onmouseout ="this.style.background='#ffffff'; this.style.bordercolor=''">
                ...
                <input type="checkbox" name="id[]" value="<?php echo $rslist['id'];?>" style="background:none; border:none;" />
                  ...
                  <td align="left" class="bline"><?php echo $rslist['username']; ?></td>
                  <td align="center" class="bline"><?php echo $rslist['mobile']; ?></td>
                  <td align="center" class="bline"><?php echo $rslist['email']; ?></td>
                  <td align="center" class="bline"><?php echo  date('Y-m-d H:i:s', $rslist['addtime']);?></td>
                  <td align="center" class="bline"><a href="member_list.php?action=member_reset_password&id=<?php echo $rslist['id']; ?>">重置密码</a></td>
                  <td align="center" class="bline"><a href="member_show.php?id=<?php echo $rslist['id'];?>">查看详情</a></td>
              </tr>	
             <?php
			}
			...
			?>
            <tr bgcolor="#ffffff">
                <td height="40" colspan="8" align="left">
  ...
  <input type="submit" name="btnSave" value="删除"/></div>
           ...
           <tr bgcolor="#ffffff">
                <td height="30" colspan="8" align="center"><?php if($count >0){ echo $key;}else{echo "暂无内容";}?></td>
           </tr>	
    </table>
    </form>
  </DIV>
